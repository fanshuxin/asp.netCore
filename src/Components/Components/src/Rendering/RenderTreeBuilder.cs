...
namespace Microsoft.AspNetCore.Components.Rendering
{
    ...
    public sealed class RenderTreeBuilder : IDisposable
    {
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(OpenElement)}");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(OpenElement)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(CloseElement)}");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(CloseElement)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddMarkupContent)}");
            if (!string.IsNullOrWhiteSpace(markupContent))
            {
                Append(RenderTreeFrame.Markup(sequence, markupContent ?? string.Empty));
            }
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddMarkupContent)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}(string)");
            if (!string.IsNullOrWhiteSpace(textContent))
            {
                Append(RenderTreeFrame.Text(sequence, textContent ?? string.Empty));
            }
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}(string)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}(RenderFragment)");
            if (fragment != null)
            {
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}(RenderFragment)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}<TValue>");
            if (fragment != null)
            {
                AddContent(sequence, fragment(value));
            }
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}<TValue>");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(bool)");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(bool)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(string)");
            AssertCanAddAttribute();
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(string)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(MulticastDelegate)");
            AssertCanAddAttribute();
            if (value != null || _lastNonAttributeFrameType == RenderTreeFrameType.Component)
            {
                Append(RenderTreeFrame.Attribute(sequence, name, value));
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(MulticastDelegate)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(EventCallback)");
            AssertCanAddAttribute();
            if (_lastNonAttributeFrameType == RenderTreeFrameType.Component)
            {
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(EventCallback)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(EventCallback<TArgument>)");
            AssertCanAddAttribute();
            if (_lastNonAttributeFrameType == RenderTreeFrameType.Component)
            {
                // Since this is a component, we need to preserve the type of the EventCallback, so we have
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(EventCallback<TArgument>)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(object)");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(object)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(RenderTreeFrame)");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(RenderTreeFrame)");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddMultipleAttributes)}");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddMultipleAttributes)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(SetUpdatesAttributeName)}");
            if (_entries.Count == 0)
            {
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(SetUpdatesAttributeName)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(SetKey)}");
            if (value == null)
            {
                ...
                TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(SetKey)}");
                return;
            }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(OpenComponentUnchecked)}");
            // We are entering a new scope, since we track the "duplicate attributes" per
            // element/component we might need to clean them up now.
            if (_hasSeenAddMultipleAttributes)
            {
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(OpenComponentUnchecked)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(CloseComponent)}");
            var indexOfEntryBeingClosed = _openElementIndices.Pop();
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(CloseComponent)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddElementReferenceCapture)}");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddElementReferenceCapture)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddComponentReferenceCapture)}");
            var parentFrameIndex = GetCurrentParentFrameIndex();
            if (!parentFrameIndex.HasValue)
            {
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(AddComponentReferenceCapture)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(OpenRegion)}");
            // We are entering a new scope, since we track the "duplicate attributes" per
            // element/component we might need to clean them up now.
            if (_hasSeenAddMultipleAttributes)
            {
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(OpenRegion)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(CloseRegion)}");
            var indexOfEntryBeingClosed = _openElementIndices.Pop();
            ref var entry = ref _entries.Buffer[indexOfEntryBeingClosed];
            entry = entry.WithRegionSubtreeLength(_entries.Count - indexOfEntryBeingClosed);
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(CloseRegion)}");
        }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(InsertAttributeExpensive)}");
            ...
            {
                TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(InsertAttributeExpensive)}");
                return;
            }
        ...
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(ProcessDuplicateAttributes)}");
            ...
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(ProcessDuplicateAttributes)} - build seenAttributeNames");
            ...
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(ProcessDuplicateAttributes)} - build seenAttributeNames");
            ...
            _hasSeenAddMultipleAttributes = false;
            TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(ProcessDuplicateAttributes)}");
        }

        // Internal for testing
        internal void TrackAttributeName(string name)
        {
            TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(TrackAttributeName)}");
            if (!_hasSeenAddMultipleAttributes)
            {
                TimingRegion.Close($"{nameof(RenderTreeBuilder)}.{nameof(TrackAttributeName)}");
                return;
            }
