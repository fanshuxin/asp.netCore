...
namespace Microsoft.AspNetCore.Components.Rendering
{
    ...
    public sealed class RenderTreeBuilder : IDisposable
    {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(OpenElement)}");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(CloseElement)}");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddMarkupContent)}");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}(string)");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}(RenderFragment)");
            if (fragment != null)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddContent)}<TValue>");
            if (fragment != null)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(bool)");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(string)");
            AssertCanAddAttribute();
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(MulticastDelegate)");
            AssertCanAddAttribute();
            if (value != null || _lastNonAttributeFrameType == RenderTreeFrameType.Component)
            {
                Append(RenderTreeFrame.Attribute(sequence, name, value));
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(EventCallback)");
            AssertCanAddAttribute();
            if (_lastNonAttributeFrameType == RenderTreeFrameType.Component)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(EventCallback<TArgument>)");
            AssertCanAddAttribute();
            if (_lastNonAttributeFrameType == RenderTreeFrameType.Component)
            {
                // Since this is a component, we need to preserve the type of the EventCallback, so we have
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(object)");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddAttribute)}(RenderTreeFrame)");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddMultipleAttributes)}");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(SetUpdatesAttributeName)}");
            if (_entries.Count == 0)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(SetKey)}");
            if (value == null)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(OpenComponentUnchecked)}");
            // We are entering a new scope, since we track the "duplicate attributes" per
            // element/component we might need to clean them up now.
            if (_hasSeenAddMultipleAttributes)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(CloseComponent)}");
            var indexOfEntryBeingClosed = _openElementIndices.Pop();
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddElementReferenceCapture)}");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(AddComponentReferenceCapture)}");
            var parentFrameIndex = GetCurrentParentFrameIndex();
            if (!parentFrameIndex.HasValue)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(OpenRegion)}");
            // We are entering a new scope, since we track the "duplicate attributes" per
            // element/component we might need to clean them up now.
            if (_hasSeenAddMultipleAttributes)
            {
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(CloseRegion)}");
            var indexOfEntryBeingClosed = _openElementIndices.Pop();
            ref var entry = ref _entries.Buffer[indexOfEntryBeingClosed];
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(InsertAttributeExpensive)}");
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(ProcessDuplicateAttributes)}");
            ...
            var buildSeenAttributeNamesDictRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(ProcessDuplicateAttributes)} - build seenAttributeNames");
            ...
            TimingRegion.Close(buildSeenAttributeNamesDictRegion);
        ...
        {
            var timingRegion = TimingRegion.Open($"{nameof(RenderTreeBuilder)}.{nameof(TrackAttributeName)}");
            if (!_hasSeenAddMultipleAttributes)
            {
                TimingRegion.Close(timingRegion);
                return;
